...
Subtract at hunk granularity, not path granularity. A path in `dirty` that the task never touched drops out whole; a path in `dirty` that the task *did* edit stays in, with the hunks already present in `baseline.diff` excluded — dropping the whole file instead would hide the task's own edits to it, which is the worse of the two errors.

Build it so nothing is silently dropped: `git diff <baseline-commit>` covers tracked edits **including staged ones**, and `git status --porcelain` names untracked files, which a plain diff omits entirely — a new file is exactly where a fresh defect hides. Write the unified diff to `.babel/<task>/inbox/changeset.diff` (untracked files appended via `git diff --no-index /dev/null <path>`) and the file list to `.babel/<task>/inbox/changeset-files.txt`. On a multi-round L run, also copy each changeset file verbatim to `.babel/<task>/snap-r<N>/<repo-relative-path>` at dispatch — A1's inter-round delta is `diff -u --strip-trailing-cr snap-r<N>/<p> <p>` per moved path, concatenated by the lead before the next dispatch. The delta is **per channel**, built from `snap-r<that channel's last_seen>` — a channel that failed, sat a round out, or was skipped by change-impact routing has an older `last_seen` and needs the wider delta. Write the common case (`last_seen == N`, the round whose snapshot was just taken) to `.babel/<task>/inbox/delta-r<N+1>.diff` and give any lagging channel its own `delta-r<N+1>-<channel>.diff`; one shared delta silently omits what the laggard never saw (`--strip-trailing-cr` because a repo whose `.gitattributes` normalizes line endings would otherwise report every line as changed). A path added since round N has no snapshot side and a path deleted since has no working-tree side: substitute `/dev/null` for the missing operand, exactly as the changeset build does for untracked files, and `cursors.<channel>.snapshot` only records which paths moved, not their content (protocol.md §5). Keep `inbox/changeset-r<N>.diff` too if you want the round on record, but do not build the delta from it: diffing two patch files produces a diff-of-diffs, not source hunks. Reviewers that can read the fs (SOL / Claude subagents) receive both by path; agy receives the hunks inline. For spec-compliance review, distribute `.babel/<task>/spec.md` the same way (inline the essentials for agy).
