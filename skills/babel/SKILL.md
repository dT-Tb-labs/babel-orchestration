...
- **Fold on measured reward, not on suspicion.** At **L, multi-round** only — the scoreboard exists nowhere else (scale-sized shapes, Phase 0) and one round of data is noise — the lead compares the split reward (`Σ 1/reporters` over confirmed records, per token; §A9) across full-width channels after each round (`channel_scoreboard`, §A9). **Measured caveat: SOL and agy report no usage, so they are `tokens: null` and this comparison cannot include them** — it governs the (a) track only, and the externals are governed by the drop rules instead (§A9). A channel an order of magnitude below the best is folded to its cheapest useful form for the next round — for an in-Claude track that means one dimension group, or moving it to the completeness-critic slot — before the §A9 drop rule would ever fire. Report the actual per-round spend against the estimate given at the crew proposal; an order-of-magnitude overshoot is itself a finding.
