...
- **On a `loop` route the cascade is the cost control** (`references/loop.md` §L2): candidates die on `git apply --check` and the frozen-set hash before anything expensive runs, and the full oracle sees only survivors. Budget the oracle at `seconds × candidates × iterations` with the candidate count counted per channel, not per iteration — three generators means three full-cascade runs each time.
