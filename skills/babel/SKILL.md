...
- **S**: Lead-only design + implementation. Acceptance = one adversarially-prompted Claude reviewer (single Agent, all four dimensions — not the (a) Workflow; see patterns.md), substituting for SOL quick.
...
2. **S/M/L classification**:
   ...
   - **S**: 2 members (lead + 1 acceptance track). Design debate omitted. Acceptance = **one Claude adversarial review** — on small diffs SOL quick tends to produce false positives, so this reduces external dependency (SOL quick is acceptable only when a SOL call is already in flight for this task — "installed and authenticated" is not the condition; Claude stays the default at S, patterns.md acceptance-gate).
...
3. **Proposal and approval gate**: Present the crew proposal (S/M/L classification + participating models + applied patterns + whether Sonnet delegation is used and its expected scope) to the user in natural language and get approval before starting (user gate "crew proposal"). **Include the estimated tokens**: expansion waves (firing a batch of parallel subagents at once) easily reach millions of tokens (measured in pilot 2), so state an order-of-magnitude like "roughly ◯ members × ◯ patterns per wave = millions of tokens" to make consumption consent explicit. Re-confirm, with the added estimated tokens attached, when a defined budget trigger fires — `rounds_consumed` reaching 4, **`round` reaching 5 whatever `rounds_consumed` says** (both: patterns.md termination condition), exceeding the SOL deep cap, or any wave shape not covered by the approved proposal. A converging round (new C/H decreased) is inside the original approval until that round-5 floor.
   ...
   - **S**: `{"round":0,"baseline":{...},"rejected":[]}` — no cursors, budget, reviewed_scope, or channel_scoreboard. S is one-shot with one reviewer (patterns.md acceptance-gate); the multi-round bookkeeping is dead weight there.
